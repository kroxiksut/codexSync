...
одной, окно начинает с экрана «Первый запуск», а все остальные страницы
предлагают открыть существующий `config.toml` из любой папки или создать новый
там, где вы скажете. Место за вас не выбирается, а существующий файл, выбранный
в поле «Файл конфигурации», открывается, а не перезаписывается.
...
проверочный запуск синхронизации, последняя синхронизация со ссылкой на её
историю и состояние задачи расписания со ссылкой на её настройки.
...
- Вкладка **История** показывает прошлые синхронизации: когда, итог, кто
  запустил, что скопировано и какая резервная копия. См.
  [Синхронизация → История](SYNC.md#история).
