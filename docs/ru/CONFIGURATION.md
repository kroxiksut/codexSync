...
jitter_seconds = 0
sync_at_login = false        # отдельная задача: синхронизировать настройки один раз после входа
```
...
- Периодическая задача выполняет только безопасное действие: `guardian_snapshot`,
  `preflight` или `sync_dry_run`. Ремонт, перенос, восстановление или откат по
  расписанию запустить нельзя, а проверочный запуск синхронизации по расписанию
  всё равно отклоняется при открытом Codex.
- **Синхронизация после входа** (`sync_at_login = true`, галка «Синхронизировать
  настройки после входа») — единственное исключение, по умолчанию выключено. Она
  ставит вторую задачу, которая один раз после входа в систему, выждав
  `startup_delay_seconds`, выполняет `sync --apply --unattended` и больше не
  повторяется. Проверки те же, что у ручной синхронизации: при открытом Codex она
  отказывается (код 3), а `--unattended` превращает любой конфликт в остановку
  до записи (код 2), что бы ни было в `conflict.policy`. Синхронизируется только
  дерево настроек — сессии задача не переносит никогда. Если Codex запускается
  вместе с Windows, к моменту запуска задачи он уже открыт, и синхронизация
  просто пропускается; уберите Codex из автозагрузки, если хотите, чтобы она
  работала. Окно показывает последний запуск задачи и что означал его результат.
