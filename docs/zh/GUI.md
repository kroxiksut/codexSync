...
`config.toml`（任何文件夹均可）或在您选择的位置新建。不会替您选定位置；在「配置文件」
一栏选中已存在的文件会直接打开它，而不是覆盖。
...
- **历史**选项卡列出过去的同步：时间、结果、启动方、复制了什么以及对应的备份。见
  [同步 → 历史](SYNC.md#历史)。
...
升级为止。这张卡片在你点开**详情**之前只占一行（当有内容挡住所有写入时会自动展开）；它会列出将要改什么、
为什么改，显示确切差异，并以一次确认过的写入
