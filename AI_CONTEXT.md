...
## 0.2 FEATURES (ALL MVP FEATURES BELOW ARE DELIVERED)
1. GUARDIAN: IMMUTABLE VERIFIED SNAPSHOTS OF THE GLOBAL STATE WHILE CODEX RUNS, WRITTEN ONLY OUTSIDE `.codex`.
2. SINGLE MUTATION AUTHORITY (`safety_gate`) AND A SINGLE MUTATION ENVELOPE: LOCK, JOURNAL, VERIFIED BACKUP, FINAL CHECK, ATOMIC REPLACE.
3. `RECOVER INSPECT|RESUME|ROLLBACK` FOR AN INTERRUPTED MUTATION.
4. `REPAIR-PROJECTS` FOR A MACHINE HANDOFF, AS AN EXACT PLAN CONFIRMED BY ITS ID.
5. `SESSIONS` SCAN/INDEX/RESOLVE/APPLY: SEMANTIC BRANCH CLASSIFICATION AND TRANSFER; A DIVERGENCE IS NEVER MERGED.
6. `CHATS` LIST/TREE/MOVE.
7. OPTIONAL GUI EXTRA `codexsync[gui]` (AND THE WINDOWED `codexsync-gui.exe`): A SECOND SHELL OVER THE SAME CORE; CORE AND CLI KEEP ZERO RUNTIME DEPENDENCIES.
...
- SYNC DIRECTION (`D-012`): `BIDIRECTIONAL|TO_CLOUD|TO_LOCAL`. A SKIPPED SIDE IS
  NEVER RECORDED AS SYNCHRONISED; THE PREVIOUS MANIFEST ENTRY IS CARRIED OVER
  UNCHANGED, AND A CONFLICT IS STILL DECIDED BY `CONFLICT.POLICY`.
- DELETE POLICY (`D-013`): `NEVER|PROPAGATE`. A DELETION TRAVELS ONLY WHERE THE
  PREVIOUS MANIFEST PROVES BOTH SIDES HELD THE FILE AND THE SURVIVING SIDE HAS
  NOT CHANGED; A VERIFIED BACKUP IS WRITTEN FIRST, INSIDE THE SAME ENVELOPE.
  SEMANTIC-OWNED PATHS ARE NEVER DELETED BY `SYNC`.
- SESSION WORKING SET: NARROWS ONLY WHAT IS WRITTEN INTO `.codex`. THE CLOUD
  MIRROR IS ALWAYS COMPLETE, AND THE SET IS PART OF THE PLAN ID.
- PREFLIGHT/DOCTOR CHECKS:
  - CONFIG + RUNTIME PATH READINESS.
  - READ ACCESS AND DIRECTORY SHAPE FOR LOCAL/CLOUD/BACKUP/TEMP ROOTS.
  ...
  - WHAT A SYNC IS ALLOWED TO DO (DIRECTION AND DELETE POLICY).
  - WHERE PROJECTS ARE STORED: JSON ONLY IS WHAT CODEXSYNC WRITES, WHILE THE
    RUNTIME ALSO KEEPS PROJECTS IN `state_*.sqlite`.
  - DOCTOR/PREFLIGHT IS SIDE-EFFECT FREE: IT NEVER WRITES A PROBE FILE, SO THE
    0.1 MTIME-DRIFT PROBE NO LONGER EXISTS.
