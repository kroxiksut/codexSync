...
Windows exe build: `codexsync.spec` + `scripts/pyinstaller_entrypoint.py` (console CLI, Qt excluded) and `codexsync-gui.spec` + `scripts/pyinstaller_gui_entrypoint.py` (windowed `CodexSync.exe`, which runs `cli.main` when given a subcommand — that is what a frozen install's scheduled task runs). Build the GUI exe with `--distpath dist/gui`: Windows treats `codexsync.exe` and `CodexSync.exe` as one file. Both are released by `.github/workflows/release-exe.yml` — `workflow_dispatch` only. CI has a second `gui` job that installs `.[gui]` and runs the GUI tests offscreen.
